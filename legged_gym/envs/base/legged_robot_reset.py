...
from .reset_design import ModifyURDF
...
class LeggedRobotReset(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.spec_design = self.cfg.designs.specified_design
        ...
        print('=' * 50)
        print('The num_envs is:',self.num_envs)
    ...
    def compute_observations(self):
        ...
        priv_latent = torch.cat((self.mass_params_tensor,
                                 self.design_params_tensor,
    ...
    def create_sim(self):
        ...
        self._assign_design(spec_design=self.spec_design)
        self._create_envs()

    def _assign_design(self,spec_design=None):
        print("*" * 80)
        print("Start creating urdf files...")
        original_filepath = "resources/lite3/urdf/Lite3+foot_changed.urdf"
        self.modify_class = ModifyURDF(original_filepath=original_filepath)
        self.asset_files = []
        self.asset_designs = []
        for i in range(self.num_envs):
            if spec_design is None:
                new_urdf, new_design = self.modify_class.randomize_design()
            else:
                new_urdf, new_design = self.modify_class.specific_design(spec_design)
            self.asset_files.append(new_urdf)
            self.asset_designs.append(new_design)
    ...
    def _create_envs(self):
        ...
        for asset_file in self.asset_files:
            assets.append(self.gym.load_asset(self.sim,'/',asset_file,asset_options))

        self.num_dof = self.gym.get_asset_dof_count(assets[0])
        self.num_bodies = self.gym.get_asset_rigid_body_count(assets[0])
        # dof_props_asset = self.gym.get_asset_dof_properties(assets[0])
        # rigid_shape_props_asset = self.gym.get_asset_rigid_shape_properties(assets[0])

        body_names = self.gym.get_asset_rigid_body_names(assets[0])
        self.dof_names = self.gym.get_asset_dof_names(assets[0])
        ...
        self.design_params_tensor = torch.zeros(self.num_envs,4,dtype=torch.float,device=self.device,requires_grad=False)

        print("Creating env...")
        for i,asset in enumerate(assets):
            '''create force sensor'''
            # method 1
            # for s in ["FL_FOOT","FR_FOOT","HL_FOOT","FR_FOOT"]:
            ...
            for name in ["FL_FOOT","FR_FOOT","HL_FOOT","HR_FOOT"]:
                ...
                index = self.gym.find_asset_rigid_body_index(asset,name)
                self.gym.create_asset_force_sensor(asset,index,sensor_pose,sensor_options)
            '''create env instance'''
            env_handle = self.gym.create_env(self.sim,env_lower,env_upper,int(np.sqrt(self.num_envs)))
            '''process and set rigid_shape_props:friction'''
            ...
            actor_handle = self.gym.create_actor(env_handle, asset, start_pose,"robot", i, self.cfg.asset.self_collisions,0)
            '''process and set actor_dof_properties: pos,vel, torque limits'''
            dof_props_asset = self.gym.get_asset_dof_properties(asset)
            dof_props = self._process_dof_props(dof_props_asset,i)
            self.gym.set_actor_dof_properties(env_handle,actor_handle,dof_props)
            '''process and set actor_rigid_body_properties: com and mass'''
            ...
            self.design_params_tensor[i, :] = torch.from_numpy(self.asset_designs[i]).to(self.device).to(torch.float)
        ...
        thigh_names = ["FL_HipY_joint","FR_HipY_joint","HL_HipY_joint","HR_HipY_joint"]
        self.thigh_indices = torch.zeros(len(thigh_names), dtype=torch.long, device=self.device, requires_grad=False)
        for i, name in enumerate(thigh_names):
            self.thigh_indices[i] = self.dof_names.index(name)
        calf_names = ["FL_Knee_joint","FR_Knee_joint","HL_Knee_joint","HR_Knee_joint"]
    ...
    def _reward_tracking_goal_vel(self):  # scales: 1.5
        norm = torch.norm(self.target_pos_rel, dim=-1, keepdim=True)
        target_vec_norm = self.target_pos_rel / (norm + 1e-5)
    ...
    def _reward_tracking_yaw(self):      # scales: 0.5
        rew = torch.exp(-torch.abs(self.target_yaw - self.yaw))
        return rew

    def _reward_lin_vel_z(self):        # scale: -1.0
        rew = torch.square(self.base_lin_vel[:, 2])
        rew[self.env_class != 17] *= 0.5
        return rew

    def _reward_ang_vel_xy(self):       # scale: -0.05
        return torch.sum(torch.square(self.base_ang_vel[:, :2]), dim=1)

    def _reward_orientation(self):     # scale: -1.0
        rew = torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1)
        rew[self.env_class != 17] = 0.
        return rew

    def _reward_dof_acc(self):        # scale: -2.5e-7
        return torch.sum(torch.square((self.last_dof_vel - self.dof_vel) / self.dt), dim=1)

    def _reward_collision(self):     # scale: -10.
        return torch.sum(1.*(torch.norm(self.contact_forces[:, self.penalised_contact_indices, :], dim=-1) > 0.1), dim=1)

    def _reward_action_rate(self):   # scale: -0.1
        return torch.norm(self.last_actions - self.actions, dim=1)

    def _reward_delta_torques(self): # scale: -1.0e-7
        return torch.sum(torch.square(self.torques - self.last_torques), dim=1)

    def _reward_torques(self):       # scale: -0.00001
        return torch.sum(torch.square(self.torques), dim=1)

    def _reward_hip_pos(self):      # scale: -0.5
        return torch.sum(torch.square(self.dof_pos[:, self.hip_indices] - self.default_dof_pos[:, self.hip_indices]), dim=1)

    def _reward_dof_error(self):    # scale: -0.04
        dof_error = torch.sum(torch.square(self.dof_pos - self.default_dof_pos), dim=1)
        return dof_error

    def _reward_feet_stumble(self): # scale: -1
    ...
    def _reward_feet_edge(self):   # scale: -1
